...
public:
	...
	bool splitAudioChannel() {
		...
		std::cout << "Finished splitting audi channels" << std::endl;
	...
	void averageOverlapWindows(std::vector<std::vector<double>> frequencyWindowMagnitudes) {
		...
		std::vector<double> secondWindow = frequencyWindowMagnitudes[window - 1];
	...
	void getSampleOverFrequency() {
		...
		if (audioChannelSplit) {
			...
			std::cout << "SampleOverFreq Runtime: " << clock.getElapsedTime().asSeconds() << " seconds " << std::endl;
		}
	}
...
private:
	...
	std::string audioFilePath = "./audio/sxfour-404.mp3";
	sf::Clock clock;
